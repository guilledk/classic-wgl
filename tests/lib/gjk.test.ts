...
describe('Shape.support', () => {
    it('returns the vertex furthest along the given direction', () => {
        ...
        expect(rect.support(versorRad(angle * 3))).toEqual(vec3.fromValues(0, 0, 0));
        expect(rect.support(versorRad(angle * 5))).toEqual(vec3.fromValues(0, 1, 0));
        expect(rect.support(versorRad(angle * 7))).toEqual(vec3.fromValues(1, 1, 0));
    });

    it('accounts for the shape position offset', () => {
        const rect = unitSquare([5, 5, 0]);
        expect(rect.support(vec3.fromValues(1, 0, 0))).toEqual(vec3.fromValues(6, 5, 0));
    });
});
...
describe('GJKContext.evolveSimplex', () => {
    it('progresses from an empty simplex without throwing', () => {
        ...
        expect([EvolveResult.StillEvolving, EvolveResult.NoIntersection]).toContain(first);
    });
