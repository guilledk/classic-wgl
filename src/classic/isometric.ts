...
export class Tilemap extends Drawable {
  ...
  rawDraw(): void {
    ...
    this.gl.uniform2fv(
      this.game.shaders.isoTilemap.unif.tileSetSize,
      this.tileSetSize
    );
    this.gl.uniform2fv(
      this.game.shaders.isoTilemap.unif.tilePixelSize,
      this.tilePixelSize
    );
    ...
    this.gl.uniform1i(
      this.game.shaders.isoTilemap.unif.selectionMode,
      this.game.selectionMode
    );
    this.gl.uniform4fv(
...
export class IsometricNavMesh extends Tilemap {
  ...
  constructor(
    ...
    this._worker = new Worker(
      new URL('./pathfinder.ts', import.meta.url),
      { type: 'module' }
    );
  ...
  update(): void {
    switch (this._state) {
      ...
      case AgentStates.followPath:
        this._delta +=
          (this.speed * this.game.deltaTime) / this._init_dist;
        if (this._delta >= 1) {
          this.nextTarget();
          this._delta = 0;
