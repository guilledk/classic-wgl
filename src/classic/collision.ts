...
export class Shape implements IShape, GJKShape {
    ...
    constructor(game: IGameState, position: Vec3Like, scale: Vec3Like, rotation: number) {
        this.game = game;
        this.gl = game.gl;
    ...
    rawDebugDraw(): void {
        ...
        this.gl.uniformMatrix4fv(this.game.shaders.solid.unif.cameraMatrix, false, mat4.create());
        this.gl.uniformMatrix4fv(
...
export class PhysicsProvider {
    ...
    beginSelection(): void {
        vec3.set(this.selection.position, this.game.mousePos[0], this.game.mousePos[1], 0);
    ...
    performCalls(): void {
        ...
        for (const c of this.screen.retrieve(this.selection)) {
            if (c._pid === 0 || !('entity' in c) || !c.entity.enabled) continue;
            const collider = c as Collider;
            if (collider.hasHandlers('selectionTemp') && this.gjk(this.selection, c)) {
    ...
    rawDebugQuadtreeDraw(currentNode: Quadtree<Collider | VirtualCollider>): void {
        ...
        this.gl.uniformMatrix4fv(this.game.shaders.solid.unif.cameraMatrix, false, mat4.create());
        this.gl.uniformMatrix4fv(this.game.shaders.solid.unif.modelMatrix, false, modelMatrix);
        this.gl.uniform4fv(
