...
class PriorityQueue<T> {
    ...
    private _siftDown(): void {
        ...
        while (
            ...
            const maxChild =
                getRightIndex(node) < this.size() &&
                this._greater(getRightIndex(node), getLeftIndex(node))
                    ? getRightIndex(node)
                    : getLeftIndex(node);
...
function aStarPath(mapName: string, from: Vec2, to: Vec2): Vec2[] | null {
    ...
    const fCostOrder = (a: Vec2, b: Vec2): boolean => fCosts.get(a) < fCosts.get(b);
...
onmessage = function (e: MessageEvent<WorkerMessage>): void {
    ...
    switch (msg.op) {
        case 'initmap': {
            ...
            console.log("Init nav mesh '" + args.name + "' of size " + args.size);
