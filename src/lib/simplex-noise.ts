...
export class SimplexNoise {
  ...
  noise3D(xin: number, yin: number, zin: number): number {
    ...
    let i1: number,
      j1: number,
      k1: number; // Offsets for second corner of simplex in (i,j,k) coords
    let i2: number,
      j2: number,
      k2: number; // Offsets for third corner of simplex in (i,j,k) coords
...
if (typeof window !== 'undefined') {
  (window as unknown as { SimplexNoise: typeof SimplexNoise }).SimplexNoise =
    SimplexNoise;
}
